    ...
    with open('data/field_pages.json', 'wt') as f, \
        ...
        for fields in grouper(read_fields(), CHUNK_SIZE):
            ...
            for field in fields:
                ...
                del field['wiki_title']
                f.write(json.dumps(field) + '\n')
                pbar.update()


def update_from_search(fields):
    ...
    for field in fields:
        if field is None:
            continue
        if field['wiki_title'] in manual_titles:
            field['wiki_title'] = manual_titles[field['wiki_title']]
    return fields


def read_fields():
    # Read our field metadata from the disk
    with open('data/fields.tsv', 'rt') as f:
