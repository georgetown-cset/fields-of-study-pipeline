...
with merged as (
  ...
  select
    merged_id,
    -- We unnest and nest again so that the structs match; non_en_imputed_scores has more nested fields
    array_agg(struct(
      field.id,
      field.score
      )) as fields,
    true as is_imputed
  from {{staging_dataset}}.non_en_imputed_scores, unnest(fields) as field
  group by merged_id
)
