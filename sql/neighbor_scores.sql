with no_pre_scored_neighbors as (
    select neighbors.merged_id,
           neighbor_id
    from {{staging_dataset}}.neighbors
             left join (select merged_id from {{staging_dataset}}.en_scores) as en_scores
                       using (merged_id)
...
select no_pre_scored_neighbors.merged_id,
       neighbor_id,
       fields
       -- For publications without field scores (see WHERE)
from (select distinct merged_id, neighbor_id from no_pre_scored_neighbors) as no_pre_scored_neighbors
